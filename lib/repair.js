 ...
 */
function renumberWithDrops(parts, dropPredicate, dropLinePredicate = null) {
  // 行身份删除（dropLinePredicate）：双流交织时保留行与删除行可能共享 seq
  // 值，按 seq 值判删会误删保留行——extractTurn 等场景必须按行身份删除。
  // 两种判据任一命中即删；dropLinePredicate 用行索引（parts 下标）。
  const dropped = new Set();
  const droppedLines = new Set();
  for (let i = 0; i < parts.length; i++) {
    const raw = parts[i];
    if (!raw.trim()) continue;
    if (dropLinePredicate && dropLinePredicate(i)) {
      droppedLines.add(i);
      continue;
    }
    let v;
...
/**
 * 按行身份保留 + 全量重编号（extract-turn / keep-ranges 用）。
 *
...
function renumberKeptLines(parts, keepLinePredicate) {
  ...
  for (let i = 0; i < parts.length; i++) {
    const raw = parts[i];
    if (!raw.trim()) continue;
    if (i === 0) { keptLineIdx.push(i); continue; } // header
    if (keepLinePredicate(i)) keptLineIdx.push(i);
  }
  // 建立 old→new 映射：保留行的展开事件按物理序分配 0,1,2,…
  const seqMap = new Map();
  let counter = 0;
  for (const li of keptLineIdx) {
    if (li === 0) continue; // header 无 seq
    const decoded = decodeLine(parts[li]);
    if (!decoded) continue;
    for (const ev of decoded) {
      if (typeof ev.seq === 'number' && !seqMap.has(ev.seq)) seqMap.set(ev.seq, counter++);
    }
  }
  const remap = (s) => (typeof s === 'number' && seqMap.has(s) ? seqMap.get(s) : null);
  // 第二遍：重写保留行
  const out = [];
  let removed = 0;
  let renumbered = 0;
  const keptSet = new Set(keptLineIdx);
  for (let i = 0; i < parts.length; i++) {
    const raw = parts[i];
    if (!raw.trim()) {
      out.push(raw);
    ...
    if (i === 0) { out.push(raw); continue; } // header 原样
    let v;
    try {
      v = JSON.parse(raw);
    } catch {
    ...
    if (CHUNK_ROW_TYPES.has(v.type) && typeof v.seq0 === 'number') {
      const ns = remap(v.seq0);
      if (ns !== null && ns !== v.seq0) {
        v.seq0 = ns;
        touched = true;
      }
    }
    if (Array.isArray(v.sourceEventSeqs)) {
      const next = v.sourceEventSeqs.map((s) => remap(s) ?? s);
      if (next.some((x, k) => x !== v.sourceEventSeqs[k])) {
        v.sourceEventSeqs = next;
        touched = true;
      }
    }
    if (v.surfaceOp && v.surfaceOp.op === 'replace') {
      const ns = remap(v.surfaceOp.start);
      const ne = remap(v.surfaceOp.end);
      if (ns !== null && ne !== null && (ns !== v.surfaceOp.start || ne !== v.surfaceOp.end)) {
        v.surfaceOp.start = ns;
        v.surfaceOp.end = ne;
        touched = true;
      }
...
export function tailRenumberText(text, startSeq, delta) {
  ...
  for (const raw of parts) {
    ...
    if (Array.isArray(v.sourceEventSeqs)) {
      let mapped;
      try {
        mapped = v.sourceEventSeqs.map((s) => {
        ...
        // 与 seq/seq0 的优雅返回一致：错误走 error 字段而非未捕获异常
        return { text, changed: 0, startSeq, delta, error: err.message };
      }
 ...
 * inboxReplayViolations 同款状态机），`start + removedCount > 队列长`
 * 即孤儿——归零后队列重放不再越界、已消费消息不再重复投递。
 *
 * @param {string} text - JSONL 全文。
...
export function neutralizeOrphanText(text) {
  const parts = text.split('\n');
  // 解析 header.seedLength（与 checks.js inboxReplayViolations 同口径）
  let seedLength = 0;
  try {
    const hdr = JSON.parse(parts[0] || '');
    if (typeof hdr?.seedLength === 'number' && Number.isSafeInteger(hdr.seedLength)) seedLength = hdr.seedLength;
  } catch { /* header 缺失时从 0 重放 */ }
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    if (v.seq < seedLength) continue; // seed 区域不重放（与官方一致）
    const splice = v.data;
    if (!splice || typeof splice.target !== 'string' || !['next-turn', 'next-step'].includes(splice.target)) continue;
    const inbox = state[splice.target];
    const removedCount = splice.removedCount ?? 0;
    const start = splice.start ?? 0;
    const inserted = Array.isArray(splice.inserted) ? splice.inserted : [];
    // 只有越界（start+removedCount > 队列长）才判为孤儿——与 I1 规则同判定
    if (start + removedCount > inbox.length && removedCount > 0) {
      v.data.removedCount = 0;
      parts[i] = JSON.stringify(v);
      seqs.push(v.seq);
      neutralized++;
      // 归零后照常推进重放状态（removedCount=0 → 插入 inserted，不删除）
      inbox.splice(start, 0, ...inserted);
    } else {
      // 合法 spliced：正常推进重放状态
      inbox.splice(start, removedCount, ...inserted);
    }
  }
  return { text: parts.join('\n'), neutralized, seqs };
}
...
export function extractTurnText(text, keepTurn, secondTurnTo = null) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (i === 0 || v.type === 'session') { kept++; continue; } // header 永远保留
    const d = v.data || {};
    const turn = d.turn;
    const t = v.type;
    let keep = false;
  ...
  // 重编号：按行身份保留 + 全量重映射 seq（从 0 连续），同步所有引用
  const r = renumberKeptLines(parts, (i) => !dropLines.has(i));
...
export function keepRangesText(text, rangesSpec) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    else dropLines.add(i);
  }
  if (keptLines === 1) return { text, removed: 0, renumbered: 0, keptLines, error: '区间未命中任何数据行（仅 header 保留）——请检查 1-based 行号' };
  const r = renumberKeptLines(parts, (i) => !dropLines.has(i));
  return { ...r, keptLines };
}
...
export function repairSession(file, opts = {}) {
  ...
  if (typeof opts.keepRanges === 'string') {
    try {
      ...
      if (r.error) {
        issues.push({ kind: 'keep-ranges', detail: r.error });
      } else {
  ...
  // 请求的修复是否"自身失败"（独立于终检）：tail-renumber 报 error、
  // keep-ranges 报 error、extract-turn 无保留行、keep-ranges 未命中——
  // 任一发生 → ok=false（修复没做成，不能报成功）。
  const requestedFailed = issues.some((i) =>
    i.kind === 'tail-renumber' && /负|错误/.test(i.detail) ||
    i.kind === 'keep-ranges' && i.detail.includes('未命中') ||
    i.kind === 'extract-turn' && i.detail.includes('不存在'));
  ...
  const ok = !requestedFailed && scanFinal.failures.length === 0 && checkFinal.ok;
  let backupPath = null;
  let applied = false;
  if (opts.apply && ok) {
