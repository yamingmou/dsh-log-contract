 ...
 * @param {{ removeMarkers?: boolean, apply?: boolean, backupDir?: string }} opts
 * @returns {{
...
export function repairSession(file, opts = {}) {
  ...
  if (opts.removeMarkers) {
    ...
    if (r.removed > 0) {
      issues.push({ kind: 'markers', detail: `移除 ${r.removed} 个 retrace/message-editor marker（重编号 ${r.renumbered} 行）` });
      plain = r.text;
      // 修复后复检
  ...
  const postBroken = issues.some((i) => i.kind.startsWith('post-'));
